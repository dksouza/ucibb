...
const predictionIndex = 0;
const distributionIndex = 1;
...
export default class GoalSummary extends PureComponent {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      //tabroutes
      index: 0,
      routes: [{ key: 'first', title: 'PREVISÃO' }, { key: 'second', title: 'DISTRIBUTION' }],
    };
  ...
  getLabel(scene) {
    const sceneLabel = scene.index === 0 ? 'PREVISÃO' : 'DISTRIBUIÇÃO';
    let labelColor;
    let labelOpacity;
  ...
  renderItemsDistributon() {
    return <BBText>ABA 2</BBText>;
  }

  render() {
    const FirstRoute = () => (
      <Container style={{ flex: 1 }}>
        <View style={{ flex: 1 }}>{this.renderItemsPrediction()}</View>
      </Container>
    );

    const SecondRoute = () => (
      <ScrollView style={{ flex: 1 }}>
        <View style={{ flex: 1 }}>{this.renderItemsDistributon()}</View>
      </ScrollView>
    );
    ...
    return (
      <BBContainer style={{ backgroundColor: BBColors.CurrentTheme.cardDefaultBg }} safebarTemplate={BBContainer.SAFE_AREA_TEMPLATE.SAFE_AREA_TEMPLATE_DEFAULT_BUTTON_SCREEN}>
        <BBStatusBar barStyle={'light-content'} />
